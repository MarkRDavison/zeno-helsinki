...
private:
    ...
    void cleanup()
    {
        ...
        vkDestroySampler(_device._device, textureSampler, nullptr);
    ...
    {
        auto swapChainSupport = hl::VulkanSwapChain::querySwapChainSupport(_device._physicalDevice, _surface.surface);
        ...
        auto queueIndices = hl::VulkanQueue::findQueueFamilies(_device._physicalDevice, _surface.surface);
    ...
    {
        auto queueFamilyIndices = hl::VulkanQueue::findQueueFamilies(_device._physicalDevice, _surface.surface);
    ...
    void createColorResources()
    {
        ...
        createImage(swapChainExtent.width, swapChainExtent.height, 1, _device._msaaSamples, colorFormat, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT | VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, colorImage, colorImageMemory);
    ...
    void createDepthResources()
    {
        ...
        createImage(swapChainExtent.width, swapChainExtent.height, 1, _device._msaaSamples, depthFormat, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, depthImage, depthImageMemory);
    ...
    void createTextureImage()
    {
        ...
        transitionImageLayout(_textureImage._image, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, mipLevels);
        copyBufferToImage(stagingBuffer, _textureImage._image, static_cast<uint32_t>(texWidth), static_cast<uint32_t>(texHeight));
        ...
        generateMipmaps(_textureImage._image, VK_FORMAT_R8G8B8A8_SRGB, texWidth, texHeight, mipLevels);
    ...
    void createTextureSampler()
    {
        ...
        if (vkCreateSampler(_device._device, &samplerInfo, nullptr, &textureSampler) != VK_SUCCESS)
        {
    ...
    VkImageView createImageView(VkImage image, VkFormat format, VkImageAspectFlags aspectFlags, uint32_t mipMapLevels)
    {
        ...
        if (vkCreateImageView(_device._device, &viewInfo, nullptr, &imageView) != VK_SUCCESS)
        {
        ...
        VkDeviceMemory& imageMemory)
    {
        ...
        if (vkCreateImage(_device._device, &imageInfo, nullptr, &image) != VK_SUCCESS)
        {
            throw std::runtime_error("failed to create image!");
        }

        VkMemoryRequirements memRequirements;
        vkGetImageMemoryRequirements(_device._device, image, &memRequirements);
        ...
        if (vkAllocateMemory(_device._device, &allocInfo, nullptr, &imageMemory) != VK_SUCCESS)
        {
            throw std::runtime_error("failed to allocate image memory!");
        }

        vkBindImageMemory(_device._device, image, imageMemory, 0);
    }

    void transitionImageLayout(VkImage image, VkFormat, VkImageLayout oldLayout, VkImageLayout newLayout, uint32_t mipMapLevels)
    {
        VkCommandBuffer commandBuffer = beginSingleTimeCommands();
    ...
    void createDescriptorSets()
    {
        ...
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++)
        {
            ...
            imageInfo.imageView = _textureImage._imageView;
    ...
    void drawFrame()
    {
        ...
        uint32_t imageIndex;
        VkResult result = vkAcquireNextImageKHR(_device._device, swapChain, UINT64_MAX, imageAvailableSemaphores[currentFrame], VK_NULL_HANDLE, &imageIndex);
